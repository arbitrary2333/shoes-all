<template>
  <el-dialog
    title="添加产品"
    :before-close="close"
    :visible.sync="addProductStatus"
    width="50%"
  >
    <el-form :model="addProductData" ref="memberRule" inline label-width="80px">
      <el-form-item
        label="商品名称"
        prop="goods_name"
        :rules="[
      ...
      >
        <el-input type="text" v-model="addProductData.goods_name"></el-input>
      </el-form-item>
      <el-form-item
        label="新价格"
        prop="newprice"
      ...
      >
        <el-input type="number" v-model="addProductData.newprice"></el-input>
      </el-form-item>
      <el-form-item
        label="旧价格"
        prop="oldprice"
        :rules="[{ required: true, message: '请输入价格', trigger: 'blur' }]"
      >
        <el-input type="number" v-model="addProductData.oldprice"></el-input>
      </el-form-item>
      <el-form-item
        label="图片"
        prop="imgurl"
      ...
      >
        <el-input v-model="addProductData.size" type="text"></el-input>
      </el-form-item>
      <el-form-item
        label="分类"
        prop="cid"
      ...
      >
        <el-select v-model="addProductData.cid" placeholder="请选择支付方式">
      ...
      >
        <el-select v-model="addProductData.bid" placeholder="请选择所属品牌">
          <el-option
      ...
      >
        <el-select v-model="addProductData.coid" placeholder="请选择商品成色">
      ...
      >
        <el-select v-model="addProductData.tid" placeholder="请选择发货时间">
      ...
      >
        <el-select v-model="addProductData.fsid" placeholder="请选择是否包邮">
    ...
    <span slot="footer" class="dialog-footer">
      ...
      <el-button type="primary" @click="addProduct('memberRule')"
        >添 加</el-button
      >
    </span>
  </el-dialog>
...
export default {
  ...
  methods: {
    handleFileChange(event) {
      ...
      this.addProductData.imgurl = file;
    },
    getBrand() {
    ...
    close() {
      this.$parent.addProductStatus = false;
    },
    addProduct(formName) {
      let self = this;
      let formData = new FormData();
      formData.append("goods_name", self.addProductData.goods_name);
      formData.append("newprice", self.addProductData.newprice);
      formData.append("oldprice", self.addProductData.oldprice);
      formData.append("imgurl", self.addProductData.imgurl);
      formData.append("size", self.addProductData.size);
      formData.append("cid", self.addProductData.cid);
      formData.append("bid", self.addProductData.bid);
      formData.append("aid", self.addProductData.aid);
      formData.append("coid", self.addProductData.coid);
      formData.append("tid", self.addProductData.tid);
      formData.append("fsid", self.addProductData.fsid);
      formData.append("aid", self.addProductData.aid);
      formData.append("coid", self.addProductData.coid);
      formData.append("tid", self.addProductData.tid);
                ...
                if (res.data.ret == 200) {
                  self.$message({
                    ...
                    onClose: function () {
                      ...
                      self.$parent.addProductStatus = false;
                      self.$parent.getList();
                    },
                  });
                } else {
